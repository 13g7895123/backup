...
func (sch *SyslogGcpScheduler) runSyslogJob(id int) {
	...
	err := sch.pool.QueryRow(ctx, `
		SELECT id, name, log_type, source_type, log_files, journal_units, journal_format,
		       dest, compress, executor_agent_id, enabled, cron_expr, last_run_at, run_status, run_message,
	...
	executor, err := resolveSyslogExecutor(ctx, sch.store, c.ExecutorAgentID)
	if err != nil {
		log.Printf("[syslog-sched] id=%d executor error: %v", id, err)
		return
	}
