...
func New(baseURL, agentCode, token string) *DashboardClient {
...
func (c *DashboardClient) GetProject(ctx context.Context, id int) (*store.Project, error) {
	var out store.Project
	return &out, c.do(ctx, "GET", fmt.Sprintf("/api/agent/projects/%d", id), nil, &out)
}
...
func (c *DashboardClient) ListTargets(ctx context.Context, projectID int) ([]store.BackupTarget, error) {
	...
	return out, c.do(ctx, "GET", fmt.Sprintf("/api/agent/projects/%d/targets", projectID), nil, &out)
}
